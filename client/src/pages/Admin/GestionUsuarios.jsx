...
function GestionUsuarios() {
    ...
    const handleEditar = (id) => {
        navigate(`/MenuPrincipal/admin/usuarios/editar/${id}`);
    };
        ...
        <div className="gestion-usuarios-page">
            <header className="gestion-header">
                ...
                <div className="tab-container">
                    <button 
                        className={`tab-btn ${rolSeleccionado === 3 ? 'active' : ''}`}
                        onClick={() => setRolSeleccionado(3)}
                    >
                        Estudiantes
                    </button>
                    <button 
                        className={`tab-btn ${rolSeleccionado === 2 ? 'active' : ''}`}
                    ...
                    <table className="sia-table">
                        <thead>
                            <tr>
                                ...
                                <th>Usuario</th>
                                <th>{rolSeleccionado === 3 ? 'Código' : 'Especialidad'}</th>
                                <th>Estado</th>
                                <th>Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            {usuarios.length > 0 ? (
                                usuarios.map(u => (
                                    <tr key={u.id_usuario}>
                                        ...
                                        <td className="acciones-celda">
                                            ...
                                            <button 
                                                onClick={() => toogleEstado(u.id_usuario, u.activo)} 
                                                className={`btn-action ${Number(u.activo) === 1 ? 'btn-disable' : 'btn-enable'}`}
                                            >
                                ...
                                <tr>
                                    <td colSpan="5" style={{ textAlign: 'center' }}>No hay usuarios registrados en este rol.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
